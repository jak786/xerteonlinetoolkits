...
function password_username_check($login_details){
	...
	require $xerte_toolkits_site->root_file_path . "languages/" . $_SESSION['toolkits_language'] . "/website_code/php/login_library.inc";

	if(($login_details["login"]=="")&&($login_details["password"]=="")){
			
		$results_and_message = array ("false", "<p>" . LOGIN_LIBRARY_PROMPT . "</p>");

		return $results_and_message;
	
	}else if($login_details["login"]==""){

		$results_and_message = array ("false", "<p>" . LOGIN_LIBRARY_PROMPT_USERNAME . "</p>");

		return $results_and_message;
	
	}else if($login_details["password"]==""){
	
		$results_and_message = array ("false", "<p>" . LOGIN_LIBRARY_PROMPT_PASSWORD . "</p>");
