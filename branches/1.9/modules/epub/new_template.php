...
function create_new_template($folder_name_id,$parent_template_name){
    ...
    if(mkdir($path)){
		mkdir($path . "/ebook");
		mkdir($path . "/ebook/chapters");
		mkdir($path . "/ebook/META-INF");
		
		copy($xerte_toolkits_site->basic_template_path . $row_framework['template_framework'] . "/container.xml" , $xerte_toolkits_site->users_file_area_full . $folder_name_id . "-" . $_SESSION['toolkits_logon_username'] . "-" . $parent_template_name . "/ebook/META-INF/container.xml");
		copy($xerte_toolkits_site->basic_template_path . $row_framework['template_framework'] . "/mimetype" , $xerte_toolkits_site->users_file_area_full . $folder_name_id . "-" . $_SESSION['toolkits_logon_username'] . "-" . $parent_template_name . "/ebook/mimetype");
