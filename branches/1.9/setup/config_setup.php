...
?>
    ...
    <li>A MYSQL Install (Xerte Online Toolkits was developed on ver 14.12 Distrib 5.05.51a for Win32. We haven't tested this with other versions, or on other database systems)
        ...
        if (!function_exists('mysql_connect')) {
            echo "<div class='error'>Your  PHP does not seem to have MySQL support </div>";
        } else {
...
?>
    <li>The root folder for this install (<?php echo $root; ?>) must be set to chmod 0777 / Full write access. 
<?php if (is_writeable($root)) {
    echo "<div class='ok'>OK</div>";
} else {
    echo "<div class='error'>Please fix</div>";
} ?>
    </li>
    <li>The setup folder for this install (<?PHP echo dirname(__FILE__); ?>) must be set to chmod 0777 / Full write access.
    ...
    <li>The user files folder for this install (<?PHP echo substr(getcwd(), 0, strlen(getcwd()) - 5) . "USER-FILES"; ?>) must be set to chmod 0777 / Full write access.
<?php if (is_writeable($root . DIRECTORY_SEPARATOR . 'USER-FILES')) {
    echo "<div class='ok'>OK</div>";
} else {
    echo "<div class='error'>Please fix</div>";
} ?>
...
<?php if (is_writeable($root . DIRECTORY_SEPARATOR . 'error_logs')) {
    echo "<div class='ok'>OK</div>";
} else {
    echo "<div class='error'>Please fix</div>";
} ?>
...
<?php if (is_writeable($root . DIRECTORY_SEPARATOR . 'import')) {
    echo "<div class='ok'>OK</div>";
} else {
    echo "<div class='error'>Please fix</div>";
} ?>
...
<h2>Further Installation Guidance</h2>
<p>
...
<ol>
    <li><b>The PHP "Short tags" setting</b> - Look in the Ini file for "short_open_tag =" and set the value to be On. At present your PHP install has this setting turned  (<?PHP if (ini_get("short_open_tag") == 1) {
    echo "<div class='ok'>on</div>";
} else {
    echo "<div class='error'>off</div>";
} ?>).</li>
